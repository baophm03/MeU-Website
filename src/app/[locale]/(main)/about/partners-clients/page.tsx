  ...
  return (
    <>
      ...
      <Section variant="white">
        <SectionHead eyebrow={t("partners.eyebrow")} title={t("partners.heading")} summary={t("partners.summary")} />
        <div className="grid gap-7 md:grid-cols-2 lg:grid-cols-4">
          {["item1", "item2", "item3"].map((item, i) => (
            <div key={item} className="border border-border p-7">
              ...
              <h3 className={cn(displayHeading, "mt-4 text-[18px] leading-[1.2]")}>{t("partners." + item + "Title")}</h3>
              <p className="mt-3 text-[13px] leading-[1.55] text-muted-foreground">{t("partners." + item + "Desc")}</p>
            </div>
          ))}
        </div>
      </Section>
